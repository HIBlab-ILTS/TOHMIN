...
def visualization():
    try:
        ...
        return render_template("data_upload.html", msg="You must upload one or more csvfile data.")
    except EmptyDataError:
        return render_template("data_upload.html", msg="Empty data files have been detected.")
    except (KeyError, TypeError, UnicodeDecodeError):
        return render_template("data_upload.html", msg="Upload valid analysis data.")
    except Exception as e:
        import traceback
        print(traceback.format_exc())
        return render_template("data_upload.html", msg=f"Error detected.: {str(e)}")
...
def input_params():
    ...
    if len(set(session["attrs"].values())) == 1:
        ...
        attr = 60
    return render_template("params_input.html", files=files, interval=attr)
...
def preview_params():
    ...
    if isinstance(tables, list):
        return render_template("params_upload.html", files=files, table=tables, header=headers)
    else:
        return render_template("params_upload.html", files=files, msg=tables)
    ...
    try:
        ...
        for file in files:
            if file in parameters_dict.keys():
                ...
                div_set |= filer.plot_coloring_events(file, folder_path, data[file], peaks)
